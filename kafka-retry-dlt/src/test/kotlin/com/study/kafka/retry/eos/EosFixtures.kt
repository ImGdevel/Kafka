 ...
 */
class ProducerGate(private val targetGroupId: String) {
	...
	fun wrap(producer: Producer<Any, Any>): Producer<Any, Any> =
		Proxy.newProxyInstance(
			...
			when (method.name) {
				...
				"commitTransaction" -> if (failing.get() && currentGroup.get() == targetGroupId) {
...
class EosApplication {
	...
	@Bean
	fun producerGate() = ProducerGate(EosListener.GROUP)
